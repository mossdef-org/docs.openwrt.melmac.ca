...
- Stops forwarded traffic during (re)starts and reloads to minimize potential WAN leaks. Note: This only stops forwarded traffic between LAN and WAN; the router itself can still connect to the internet. 
...
| Hidden         | <a name="procd_reload_delay"></a>procd_reload_delay                | integer        | 0              | Introduces a pause (in seconds) during service reload, which can help mitigate race conditions when multiple network configuration changes happen simultaneously. It controls the delay time of the triggers that are automatically set on all supported interfaces and on configuration changes of PBR and Network. If interfaces are slow to come up (e.g., OpenVPN), or if they depend on Network configuration changes, increasing this value to 5 or higher may be necessary.                                                                                                                                                              |
